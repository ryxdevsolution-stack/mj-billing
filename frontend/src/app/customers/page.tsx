      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm sm:text-base text-gray-600">Manage your customer base and track revenue</p>
          </div>
          <Link
            href="/billing/create"
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-2.5 text-sm sm:text-base border border-gray-300 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-md p-8 sm:p-12 text-center">
            ...
            <p className="text-base sm:text-lg text-gray-500">No customers found</p>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Bills</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredCustomers.map((customer) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm font-medium text-gray-900">{customer.total_bills}</div>
                          <div className="text-xs text-gray-500">
                            {customer.gst_bills} GST, {customer.non_gst_bills} Non-GST
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-sm font-bold text-green-600">{formatCurrency(customer.total_amount)}</div>
                          <div className="text-xs text-gray-500">
                            Avg: {formatCurrency(customer.total_amount / customer.total_bills)}
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-900">{formatDate(customer.last_purchase)}</td>
                        <td className="px-6 py-4">
            ...
            <div className="md:hidden space-y-3">
              {filteredCustomers.map((customer) => (
                <div
                  key={customer.customer_phone}
                  onClick={() => setSelectedCustomer(customer)}
                  className="bg-white rounded-xl shadow-md p-4 hover:shadow-lg transition touch-manipulation"
                >
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-100">
                    <div>
                      ...
                      <p className="text-base font-bold text-gray-900">{customer.total_bills}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500">Total Spent</p>
                      <p className="text-base font-bold text-green-600">{formatCurrency(customer.total_amount)}</p>
                    </div>
                    <div className="col-span-2">
                      <p className="text-xs text-gray-500">Last Purchase</p>
                      <p className="text-sm text-gray-900">{formatDate(customer.last_purchase)}</p>
                    </div>
                  </div>
                </div>
              ))}
