...
export default function GSTBillingPage() {
  ...
  const calculateGSTAmount = () => {
    return (calculateSubtotal() * formData.gst_percentage) / 100
  }
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Create GST Bill</h1>
        <p className="mt-2 text-gray-600">Generate a new GST bill with automatic tax calculation</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="border-t pt-4 space-y-2">
            ...
            <div className="flex justify-between text-lg">
              <span className="text-gray-700">GST ({formData.gst_percentage}%):</span>
              <span className="font-semibold">₹{calculateGSTAmount().toFixed(2)}</span>
            </div>
        ...
        <div className="flex gap-4">
          <button
            type="submit"
            disabled={loading}
            className="flex-1 px-8 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition disabled:bg-gray-400 text-lg font-semibold"
          >
            {loading ? 'Creating Bill...' : 'Create GST Bill'}
          </button>
          <button
            type="button"
