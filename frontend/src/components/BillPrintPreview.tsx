...
import { qzPrinter, browserPrint, downloadPdf, shareWhatsApp, BillData, ClientInfo } from '@/lib/qzPrinter'
...
export default function BillPrintPreview({ bill, clientInfo, onClose, autoPrint = false }: BillPrintPreviewProps) {
  ...
  // Direct print function - Uses QZ Tray if available, otherwise falls back to browser print
  const handleDirectPrint = async () => {
    if (qzAvailable) {
      await handleQzPrint()
    } else {
      // For cloud/SaaS mode, use browser print as fallback
      try {
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 print:hidden">
        <div className="bg-white rounded-lg shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="border-t border-gray-200 p-4 bg-white print:hidden">
            ...
            {/* QZ Tray Available - Single Direct Print Button */}
            {qzAvailable && (
              <button
                type="button"
                onClick={() => {
              ...
              >
                {isPrinting ? (
                  ...
                  <>
                    ...
                    <span>Print Bill (Silent)</span>
                  </>
                )}
              </button>
            )}

            {/* Cloud Mode - Multiple Print Options */}
            {!qzAvailable && qzAvailable !== null && (
              <div className="space-y-2">
                ...
                <div className="pt-2 text-center">
                  <p className="text-xs text-gray-500 mb-1">For silent thermal printing without dialogs:</p>
                  <a
            ...
            {qzAvailable === null && (
              <button
                type="button"
                disabled
                className="w-full px-6 py-3 bg-gray-400 cursor-not-allowed text-gray-200 rounded-lg font-medium flex items-center justify-center gap-2"
              >
                <svg className="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
